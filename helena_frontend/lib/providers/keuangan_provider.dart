...
class KeuanganProvider with ChangeNotifier {
  ...
  // --- FUNGSI BAYAR UKT ---
  // Versi baru: cukup satu panggilan API ke backend.
  // Backend yang sekarang bertanggung jawab atas atomicity:
  //   - Kalau pencatatan transaksi di pocket-money-service gagal,
  //     backend otomatis rollback tagihan sebelum balas ke kita.
  //   - Flutter tidak perlu tahu detailnya -- sukses atau gagal total.
  Future<bool> bayarUKTReal(String nim, int nominalUKT) async {
    try {
      ...
      final resBayar = await http.put(
        ...
        headers: headers,
      ).timeout(const Duration(seconds: 20)); // lebih panjang karena backend
                                               // juga menunggu pocket-money-service
